...
public class CustomAccessPathBasedSourceSinkManager extends AccessPathBasedSourceSinkManager {
	private Set<String> stringSourcesSigs;
	private Set<String> fieldSourceSigs;
	private Set<String> stmtSourceSigs;

	public CustomAccessPathBasedSourceSinkManager(Set<SourceSinkDefinition> sources, Set<SourceSinkDefinition> sinks,
	...
	private boolean isStmtSource(Stmt sCallSite) {
	...
	private boolean isStringSource(Stmt sCallSite) {
		...
		for (ValueBox ccVB : checkConstUseBoxes) {
			if (ccVB.getValue() instanceof StringConstant) {
				...
				if (this.stringSourcesSigs.contains(strV)) {
					return true;
				}
			}
		}
	...
	public boolean isSource(Stmt sCallSite) {
		return (this.isStringSource(sCallSite) || this.isStmtSource(sCallSite)
				|| this.isFieldSource(sCallSite));
	}
	...
	public SourceInfo getSourceInfo(Stmt sCallSite, InfoflowManager manager) {
		...
		if (ret != null) {
			...
			if (Globals.EXTRA_SOURCE_FILTER.shouldIgnoreSource(sCallSite, srcMethod.getDeclaringClass().getName())) {
				Log.dumpln(Globals.LOG_FILE, "SOURCE BLOCKED: " + sCallSite.toString() + " in " + srcMethod.getSignature());
				return null;
			}
			...
			if (sCallSite.containsInvokeExpr() &&
					...
					if (varg0 instanceof Local) {
						...
						SourceSinkDefinition def = new StatementSourceSinkDefinition(sCallSite,
								local, apTuple);
		...
		try {
			if (sCallSite instanceof DefinitionStmt) {
				...
				if (lhs instanceof FieldRef) {
					...
					SourceSinkDefinition def = new FieldSourceSinkDefinition(lhsField.getSignature());
					...
					if (local != null) {
						Set<AccessPathTuple> apTuple = new HashSet<AccessPathTuple>();
						apTuple.add(AccessPathTuple.create(true, false));
						SourceSinkDefinition def = new StatementSourceSinkDefinition(sCallSite, local, apTuple);
				...
				if (sCallSite.getInvokeExpr() instanceof InstanceInvokeExpr) {
					...
					Set<AccessPathTuple> apTuple = new HashSet<AccessPathTuple>();
					apTuple.add(AccessPathTuple.create(true, false));
					SourceSinkDefinition def = new StatementSourceSinkDefinition(sCallSite, lLocal, apTuple);
					this.sourceStatements.put(sCallSite, def);
